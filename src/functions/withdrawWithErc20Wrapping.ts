...
export async function withdrawNativeTokenWithErc20Wrapping(
  ...
  // the first call: get estimated amounts
  let amounts = await depositGuardContract.callStatic.withdrawFromICHIVaultAndTryUnwrapToERC20AndForwardNative(
    vaultAddress,
    vaultDeployerAddress,
    withdrawShares,
    accountAddress,
  ...
  const gasLimit =
    overrides?.gasLimit ??
    calculateGasMargin(
      await depositGuardContract.estimateGas.withdrawFromICHIVaultAndTryUnwrapToERC20AndForwardNative(
        vaultAddress,
        vaultDeployerAddress,
        withdrawShares,
        accountAddress,
